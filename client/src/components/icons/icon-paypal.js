import React from "react";

const IconPaypal = props => (
  <svg
    version="1.1"
    id="Capa_1"
    xmlns="http://www.w3.org/2000/svg"
    x="0px"
    y="0px"
    viewBox="0 0 512 512"
    style={{enableBackground:"new 0 0 512 512"}}
  >
    <path
      style={{fill:"#ECEFF1"}}
      d="M480,80H32C14.368,80,0,94.368,0,112v288c0,17.664,14.368,32,32,32h448c17.664,0,32-14.336,32-32V112
	C512,94.368,497.664,80,480,80z"
    />
    <path
      style={{fill:"#2196F3"}}
      d="M360.128,198.496C347.968,184.192,330.176,176,311.36,176H224c-7.84,0-14.496,5.664-15.776,13.376
	l-32,192c-0.768,4.64,0.544,9.376,3.584,12.96C182.816,397.952,187.296,400,192,400h32c7.84,0,14.496-5.664,15.776-13.376
	L253.536,304h57.824c31.424,0,57.984-22.432,63.136-53.408C377.568,231.872,372.32,212.864,360.128,198.496z"
    />
    <path
      style={{fill:"#1976D2"}}
      d="M328.128,166.496C315.968,152.192,298.176,144,279.36,144H192c-7.84,0-14.496,5.664-15.776,13.376
	l-32,192c-0.768,4.64,0.544,9.376,3.584,12.96C150.816,365.952,155.296,368,160,368h32c7.84,0,14.496-5.664,15.776-13.376
	L221.536,272h57.824c31.424,0,57.984-22.432,63.136-53.408C345.568,199.872,340.32,180.864,328.128,166.496z"
    />
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
    <g></g>
  </svg>
);

export default React.memo(IconPaypal);
